...
export async function getPosts() {
    return openDb().then(db => {
        return db.all('SELECT * FROM Posts order by created_at desc' ).then(res => res)
    });
}
...
export async function updatePost(_post) {
    console.log(_post);
    if (_post.title.match(/.*\S.*/) && _post.content.match(/.*\S.*/)) {
        let updated_at = new Date();
        openDb().then(db => {
            db.run('UPDATE Posts SET title = ?, subtitle = ?, content = ?, updated_at = ? WHERE id = ?',
                [_post.title, _post.subtitle, _post.content, updated_at, _post.id])
        });
    } else {
        throw new Error("Entrada inválida, tente novamente.");
    }
